...
case class NestedDisconnectRelationInterpreter(mutaction: NestedDisconnectRelation) extends NestedRelationInterpreterBase {
  ...
  override def requiredCheck(implicit mutationBuilder: OracleApiDatabaseMutationBuilder) =
  ...
  override def removalActions(implicit mutationBuilder: OracleApiDatabaseMutationBuilder) = List(removalByParentAndChild)

  override def addAction(implicit mutationBuilder: OracleApiDatabaseMutationBuilder) = noActionRequired
}
