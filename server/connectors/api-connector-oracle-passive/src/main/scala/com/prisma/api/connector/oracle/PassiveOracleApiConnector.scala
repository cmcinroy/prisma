...
import com.prisma.api.connector.oracle.database.{Databases, OracleApiDatabaseMutationBuilder, OracleDataResolver}
import com.prisma.api.connector.oracle.impl._
...
case class PassiveOracleApiConnector(config: DatabaseConfig)(implicit ec: ExecutionContext) extends ApiConnector {
  lazy val databases          = Databases.initialize(config)
  val activeConnector         = OracleApiConnector(config)
  ...
  override def dataResolver(project: Project)                       = OracleDataResolver(project, databases.readOnly, schemaName = config.schema)
  override def masterDataResolver(project: Project)                 = OracleDataResolver(project, databases.readOnly, schemaName = config.schema)
...
case class PassiveDatabaseMutactionExecutorImpl(activeExecutor: OracleDatabaseMutactionExecutor, schemaName: Option[String])(implicit ec: ExecutionContext)
  ...
  override def execute(mutactions: Vector[DatabaseMutaction], runTransactionally: Boolean): Future[Vector[DatabaseMutactionResult]] = {
    ...
    val mutationBuilder = OracleApiDatabaseMutationBuilder(
...
case class NestedCreateDataItemInterpreterForInlineRelations(mutaction: NestedCreateDataItem) extends DatabaseMutactionInterpreter {
  ...
  override def action(mutationBuilder: OracleApiDatabaseMutationBuilder): DBIO[Unit] = {
  ...
  import com.prisma.api.connector.oracle.database.SlickExtensions._

  def queryParentAndCreateDataItem(mutationBuilder: OracleApiDatabaseMutationBuilder) = {
  ...
  def createDataItemWithLinkToParent(mutationBuilder: OracleApiDatabaseMutationBuilder)(parentId: String) = {
